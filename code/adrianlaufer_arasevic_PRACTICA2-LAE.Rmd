...
```
Los resultados de las gráficas QQ nos indican que la varibale precio podria ser candidata a seguir una distribución normal. Por otra parte, los datos de la variable "rooms" y "surface" se alejan siginificantivamente de la diagonal con lo cuál probablemente no sigan una distribución normal.
...
El test de Shapiro asume como hipótesis nula que la población está distribuida normalmente. Por lo tanto, cuando el valor p es menor que el nivel de significancia (normalmente $\alpha$=0.05), la hipótesis nula es rechazada y se puede decir que los datos no cuentan con una distribución normal. Como se puede ver en los tres test realizados, ninguna de las tres variables sigue una distribución normal.

## Comprobación de la homocedasticidad

Tal y como se pide en el enunciado, realizaremos también una comprobación de la homocedasticidad en los datos, es decir la igualdad de varianzas entre los grupos. Cuando los datos utilizados no siguen una distribución normal, el test más útil para realizar la comprobación de la igualdad de las varianzas es el ede Flignet-Killeen.

A continuación realizamos el test de Flignet-Killeen con algunas variables y vemos que no se cumple la homocedasticidad de los datos, puesto que las pruebas obtenidas tienen un p-valor inferior al nivel de siginificancia de $\alpha$=0.05.
...
Antes de aplicar una regresión lineal al conjunto de datos, con el objetivo de aproximar la relación de dependencia entre dos variables, veremos como están distribuidos los datos en el conjunto: 

```{r message= FALSE, warning=FALSE}
table(fotocasa.df$rooms, fotocasa.df$tipo_vivienda)
```

Vemos, que la mayoria de immuebles son del tipo "Piso".
...
```{r message= FALSE, warning=FALSE}
aggregate(cbind(price,surface, "€/m2" = price/surface) ~ rooms , data = fotocasa.df, FUN = mean)
```
De la tabla de agregación vemos que el precio medio y la superficie media de las viviendas aumenta en función del número de habitaciones, cosa que parece lógica. Cabria mencionar, que en el caso de 6 habitaciones, tanto el precio medio de la vivienda como el número de metro cuadrados medio disminuye en comparación a los pisos de 5 habitaciones. Esto se debe probablemente a algún valor extremo y a la reducida cantidad de viviendas con tal número de habitaciones, lo cuál reduce la significancia de la media calculada.
...
aggregate(cbind(price,surface, "€/m2" = price/surface) ~ rooms , data = pisos, FUN = mean)
```
...
aggregate(cbind(price,surface, "€/m2" = price/surface) ~ rooms , data = casas, FUN = mean)
```
...
A continuación nos centraremos por lo tanto en los pisos, para poder determinar si el precio de la vivienda se ajusta a una función lineal en función de la superficie del piso.
...
A continuación creamos un modelo de regresión lineal a partir del cual queremos determinar el precio en función de la superfície del piso y entrenado a partir de los datos previamente seleccionados "pisos.entrenamiento".
```{r message= FALSE, warning=FALSE}
model <- lm(formula=price ~ surface, data=pisos.entrenamiento)
summary(model)
```
En este caso el coefciente R^2 es relativamente bajo `r summary(model)$r.squared`
...
results <- data.frame(pisos.test$price, predicted=estimated.price, dif= (pisos.test$price-estimated.price)*100/pisos.test$price)
colnames(results)<- c("Real", "Predecido","Dif%")
...
Visto que el modelo no es del todo bueno, al estar R^2 alejado de 1, realizamos un segundo modelo de regesión lineal. Tal y como hemos visto en una de las gráficas superiores, vemos que para los pisos de 1 habitación y de más de 4 habitaciones, la correlación lineal entre el precio prácticamente no existe. Por ello consideraremos para el siguiente modelo sólo los pisos de 2, 3 y 4 habitaciones.
```{r}
...
```{r message= FALSE, warning=FALSE}
model2 <- lm(formula=price ~ surface + parking + swimming_pool + terrace + balcony, data=pisos2.entrenamiento)
summary(model2)
```

Vemos de los resultados que en este caso se ha mejorado ligeramente el valor R^2, ya que este es igual a `r summary(model2)$r.squared`.
...
Parece interesante ver que el primer coeficiente que determina la relación entre la superficie y el precio igual a `coef(model2)$surface`es cercano a la media de los precios por metro cuadrado establecido anteriormente. Los otros coeficientes determinan en que medida afectaria cada propiedad al precio de la vivienda. Resaltar por lo tanto que una plaza de parking tendria una mayor influencia según el modelo calculado.

Finalmente realizamos una segunda predicción a raiz del segundo modelo calculado.
```{r message= FALSE, warning=FALSE}
...
Vemos que en este caso, la media del error entre los valores reales y el predecido disminuye ligeramente al aumentar la calidad del modelo de regresión lineal.
...
## Influencia del tipo de vivienda sobre el precio
...
ggplot() +
  ...
  geom_point(data = results, aes(surface, Predecido), color = "red", size = 2) +
  theme_minimal() +
  labs(x = "Superficie",
    y = "Precio",
