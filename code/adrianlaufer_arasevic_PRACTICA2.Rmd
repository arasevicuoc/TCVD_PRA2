...
En primer lugar importaremos el archivo `output_fotocasa.csv` con los dato del web scraping del portal www.fotocasa.es
...
A continuación importamos los datos extraidos del portal el idealista y vemos con la función summary como están compuestos
...
## Agregación de los datasets y selección de datos
...
```{r message= FALSE, warning=FALSE}
fotocasa.pisos <- filter(data2, source=="Fotocasa" & tipo_vivienda=="Piso")
fotocasa.casas <- filter(data2, source=="Fotocasa" & tipo_vivienda=="Casa")
```
...
A continuación vamos a analizar los valores extremos mediante un boxplot para ver 
```{r message= FALSE, warning=FALSE}
fotocasa.bp <- boxplot(fotocasa.pisos$price)
fotocasa.bp$out
for (x in fotocasa.bp$out) {
  print(subset(fotocasa.pisos, price == x))
}
```


```{r message= FALSE, warning=FALSE}
fotocasa.pisos <- fotocasa.pisos[-563,]
```

```{r message= FALSE, warning=FALSE}
fotocasa.bp2 <- boxplot(fotocasa.pisos$surface)
fotocasa.bp2$out
for (x in fotocasa.bp2$out) {
  print(subset(fotocasa.pisos, surface == x))
}
```
...
```{r message= FALSE, warning=FALSE}
fotocasa <- filter(data2, source=="Fotocasa")
fotocasa_num <- subset(fotocasa, select=c(rooms, price, surface))
#cor(fotocasa_num)

#cor.mat <- round(cor(fotocasa),2)
#cor.mat
...
```{r message= FALSE, warning=FALSE}
library(ggplot2)
a <- data2 %>% filter(data2$tipo_vivienda == "Piso")
ggplot(data = a,aes(x=surface,y=price, color=rooms))+
...
```{r message= FALSE, warning=FALSE}
model <- lm(price ~ surface, subset(filter(fotocasa, tipo_vivienda == "Piso"), select=c(price, surface)))
summary(model)
```
